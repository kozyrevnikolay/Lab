...
# TODO: 5) Make a reconnect button IPS120.query('C3')
# TODO: in order to reconnect IPS from computer if one has switched control to local.
...
HEATER = {'0': 'Off zero',   # Heater is off, magnet at zero
          ...
          '3': '3',
          '4': '4',          # This flag appears once in status when the heater is being switched off
...
class IPS120(SerialClass):
    """
    This class allows to use the IPS120 equipment. Contains the methods which allow to control one and a GUI. Can be
    ...
    def __init__(self):
        ...
        # self.query('W')  # Asks IPS120 to send data more slowly. Solution for slow PC.
        ...
        self.gui = MainWindow()
        self.init_values()
        self.gui.reconnect_action.triggered.connect(self.reconnect)
    ...
    def heater_on_off(self):
        with self.query_thread.pause():
            if 'Off' in self.heater:
                ...
                self.query('H0')
                # Sleep for a second after the Heater button is pressed off. Otherwise the program crashes.
                time.sleep(1)
